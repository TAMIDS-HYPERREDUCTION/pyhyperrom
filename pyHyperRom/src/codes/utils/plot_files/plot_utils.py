...
def e_n_2ij(dim, iel, ncells):
    # you can use node number in place of iel, and write el=False.
    indices = []
    divisor = 1
    
    for d in range(dim):  # iterate from last dimension to first
        size = ncells[d]  # get size of current dimension
...
class ThreeDPlot:
    ...
    def element3D(self, grid_size, xi, cells, fsize=(8, 8)):
        ...
        for i in range(len(mask_xi)):
            if mask_xi[i]:
                x, y, z = e_n_2ij(3,i,cells)  # Convert flat index to 3D index
...
class TwoDPlot:
    ...
    def contour_(self):
        ...
        fig, ax = plt.subplots()
        cp = ax.contourf(X, Y, Z, cmap=cm.coolwarm,levels=np.linspace(np.min(Z), np.max(Z), num=32))
        ax.set_xlabel('$x$')
        ax.set_ylabel('$y$')
        ax.locator_params(nbins=self.max_ticker)
    ...
    def element_(self, grid_size, xi, cells, highlight_color='#169C9A', clr='orange'):
        """
...
class OneDPlot:
    ...
    def scatter_(self):
        ...
        self.ax.set_ylabel('$T$', labelpad= 1, loc='center')

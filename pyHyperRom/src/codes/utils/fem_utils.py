...
def solve_fos(self, sol_init, tol=1e-5, max_iter=300, op=False):
    ...
    Jac, res, Ke, rhs_e, _ = eval_resJac(self, mask, self.dir_nodes, self.sol_dir, sol, node_eqnId)
    ...
    while (it < max_iter) and not(norm_ < tol):
        ...
        Jac, res, Ke, rhs_e, rhs = eval_resJac(self, mask, self.dir_nodes, self.sol_dir, sol, node_eqnId)

...
def weighted_matrix_assembly_deim(rom_cls, mask, dir_nodes, sol_dir, T_red, node_eqnId, xi, V):
    ...
    for iel in range(rom_cls.data.n_cells):
        ...
        if (xi[iel] != 0):
            ...
            # elif np.isin(0, elem_glob_node_eqnId):
            #     Ke_, _, qe_ = compute_element_matrices(rom_cls, sol_prev.flatten(), iel)
            # else:
            #     _, _, qe_ = compute_element_matrices(rom_cls, sol_prev.flatten(), iel)

            # # Update global stiffness and Jacobian matrices with weighted local matrices
            # K_r += V[col_indices].T @ (Ke_[i_index, j_index]) @ V[col_indices]
            # J_r += V[col_indices].T @ (Je_[i_index, j_index]) @ V[col_indices]

            # # Effect of the mean
            # K_r_mean += V[col_indices].T @ (Ke_[i_index, j_index]) @ T_mean[col_indices]
    ...
    M = rom_cls.deim_cls.deim_mat
    deim_mask = rom_cls.deim_cls.bool_sampled
    KT = K @ sol_prev[rom_cls.data.mask]

    # rhs_bc = np.zeros_like(rhs)
    # rhs_bc[-1] = np.copy(rhs[-1])

    residual = V.T @ KT - np.dot(M, rhs[deim_mask]) #- V.T @rhs_bc

    # residual = np.dot(M, KT[deim_mask]- rhs[deim_mask]) #- V.T @rhs_bc
...
class rom_deim(FOS_FEM):
    ...
    #     K_r, J_r, _, res = weighted_matrix_assembly(self, mask, dir_nodes, sol_dir, T_red, node_eqnId, xi, V,deim=True)
    #     return K_r + J_r, res
    
    def eval_res_fsolve_rom(self, mask, dir_nodes, sol_dir, T_red, node_eqnId, xi, V):

       return weighted_matrix_assembly_deim(self, mask, dir_nodes, sol_dir, T_red, node_eqnId, xi, V)

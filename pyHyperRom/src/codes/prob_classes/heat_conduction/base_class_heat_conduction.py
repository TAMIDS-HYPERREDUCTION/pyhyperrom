...
class FOS_FEM:
    ...
    def element_matrices(self, T_prev, iel, affine=False):
        ...
        if not self.ecm:
            ...
            # Temporary matrices and vectors for each Gauss point
            Ke_gp = np.zeros((len(self.w), n, n))  # Store stiffness matrices for each Gauss point
            Je_gp = np.zeros((len(self.w), n, n))  # Store Jacobian matrices for each Gauss point
            qe_gp = np.zeros((len(self.w), n))     # Store source vectors for each Gauss point


            for gp in range(len(self.w)):  # Gauss point loop
            ...
            # Sum up Gauss point contributions with their weights to get final matrices
    ...
    def residual_func_ecm(self,i,j,k,p_sol,data_ecm):
        ...
        K_mus_ij = K_mus[i][j][k] # k-th gaussian point
...
class HeatConductionSimulationData:
    ...
    def __init__(self, n_ref, L, quad_deg=3, num_snapshots=15, pb_dim=1, params = np.arange(1., 4.0, 0.01), T_init_guess = 273.0, train_mask=None, test_mask=None, ecm = False):
    ...
    def run_simulation(self):
        ...
        for i in range(self.num_snapshots):
            print(f"Snap {i}")
            param = self.params[i]#random.choice(self.params)  # Choose from parameter list
...
class HeatConductionSimulationData_affine:
    ...
    def __init__(self, n_ref, L, quad_deg=3, num_snapshots=15, pb_dim=1, params = np.arange(1., 4.0, 0.01),train_mask=None, test_mask=None):

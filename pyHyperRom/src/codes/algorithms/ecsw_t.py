...
def ecsw_red(d, V_sel, Le, K_mus, C_mus, q_mus, n_sel, N_snap, mask, NL_solutions, tol=None):
    """
    Function: ecsw_red
    ...
    for i in range(N_snap):
        ...
        projected_sol_mask_d = np.dot(P_sel, NL_solutions[:dim,i])
        projected_sol_mask_v = np.dot(P_sel, NL_solutions[dim:,i])
        ...
        for j in range(ncells):
            ...
            F_e = np.dot(K_mus_ij, projected_sol_mask_d[col_indices]) + np.dot(C_mus_ij, projected_sol_mask_v[col_indices]) - q_mus_ij

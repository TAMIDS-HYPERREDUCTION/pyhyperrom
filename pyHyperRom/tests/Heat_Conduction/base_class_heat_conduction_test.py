...
class HeatConductionSimulationData:
    ...
    def __init__(self, n_ref, L, quad_deg=3, num_snapshots=15, pb_dim=1, params = np.arange(1., 4.0, 0.01),T_init_guess=2.0):
    ...
    def process_snapshot(self, i, param):
        ...
        if i == 0:
            ...
            tic_fos = time.time()
            NL_solution_p, Ke, rhs_e, _, rhs_ = solve_fos(FOS, T_init)
            toc_fos = time.time()
        ...
        return (toc_fos - tic_fos, NL_solution_p.flatten(), rhs_, Ke, rhs_e)


    def run_parallel(self):
        
            # Handle the i == 0 case outside of the parallel loop
            first_param = random.choice(self.params)
            self.param_list.append(first_param)
            fos_time_0, NL_solution_p_0, rhs_0, Ke_0, rhs_e_0 = self.process_snapshot(0, first_param)
    
            # Preallocate memory for the results
            results = []#[None] * (self.num_snapshots - 1)
    
            # Use a ProcessPoolExecutor to parallelize the loop for the remaining iterations
            with concurrent.futures.ProcessPoolExecutor() as executor:
                # Schedule the function to be called on each iteration
                futures = {executor.submit(self.process_snapshot, i, random.choice(self.params)): i for i in range(1, self.num_snapshots)}
                ...
                for future in concurrent.futures.as_completed(futures):
                    ...
                    try:
                        ...
                        self.param_list.append(self.params[i])  # This may need to be adjusted depending on how params are used
                    except Exception as e:
                        print(f"Snapshot {i} generated an exception: {e}")
    
            # Insert the first result into the correct position
            self.fos_time = [fos_time_0] + [result[0] for result in results]
            self.NL_solutions = [NL_solution_p_0] + [result[1] for result in results]
            self.rhs = [rhs_0] + [result[2] for result in results]
            self.K_mus = [Ke_0] + [result[3] for result in results]
            self.q_mus = [rhs_e_0] + [result[4] for result in results]
